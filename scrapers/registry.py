...
def get_scraper(source: dict[str, Any]) -> BaseScraper:
    ...
    if cls is None:
        raise ValueError(f"Unknown scraper type: {source_type!r}. Valid types: {list(_REGISTRY)}")

    name = source["name"]
    url = source["url"]
    params = source.get("params", {})

    # ATS-specific required kwargs
    kwargs: dict[str, Any] = {"params": params}
    if source_type == "greenhouse":
        token = params.get("token") or source.get("token")
        if not token:
            raise ValueError(f"Greenhouse source {name!r} requires params.token")
        kwargs["token"] = token
    elif source_type == "lever":
        company = params.get("company") or source.get("company")
        if not company:
